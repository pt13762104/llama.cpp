...
extra_buffer_map_t extra_buf_map;
struct test_case {
    virtual ~test_case() {}
    ...
    bool eval_perf(ggml_backend_t backend, const char * op_names_filter, printer * output_printer) {
        ...
        ggml_build_forward_expand(gf, out);
        if (extra_buf_map.size()) {
            auto a = ggml_backend_graph_copy(backend, gf, extra_buf_map);
            gf = a.graph;
        }
        // warmup run
...
int main(int argc, char ** argv) {
    ...
    for (size_t i = 0; i < ggml_backend_dev_count(); i++) {
        ...
        if (backend_filter == NULL && ggml_backend_dev_type(dev) == GGML_BACKEND_DEVICE_TYPE_CPU && mode == MODE_TEST) {
